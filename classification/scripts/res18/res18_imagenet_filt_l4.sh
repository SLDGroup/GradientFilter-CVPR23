...
model_args="--model.set_bn_eval True --model.use_sgd True --model.learning_rate 0.05 --model.lr_warmup 4 --model.num_classes 1000 --model.momentum 0.9 --model.anneling_steps 90 --model.scheduler_interval epoch --model.init_lr_prod 0.1"

trainer_args="--trainer.max_epochs 90 --trainer.gpus 4 --trainer.strategy ddp --trainer.gradient_clip_val 2.0 "

logger_args="--logger.save_dir runs/cls/res18/imagenet"

common_args="$trainer_args $data_args $model_args $logger_args $load_args"

python trainer_cls.py --config configs/cls/res18/filt_last4_r2.yaml ${common_args} --logger.exp_name filt_l4_${usr_group_kl} --model.with_grad_filter True
