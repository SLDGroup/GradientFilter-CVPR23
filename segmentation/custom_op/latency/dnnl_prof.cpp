...
public:
    ...
    Conv(engine &eng, memory::dims x_shape, int c_out,
         bool train_w, bool train_x,
         int conv_stride, int kern_size): x_shape(x_shape), train_w(train_w),
         train_x(train_x), conv_stride(conv_stride), kern_size(kern_size) {
        w_shape = {c_out, x_shape[1], kern_size, kern_size};
        b_shape = {c_out};
        y_shape = {x_shape[0], c_out, x_shape[2] / conv_stride, x_shape[3] / conv_stride};
        ...
        if (train_w || train_x) {
            auto conv_fwd_desc = convolution_forward::desc(
                ...
                x_md, w_md, b_md, y_md, stride, padding, padding);
            conv_fwd_pd = convolution_forward::primitive_desc(conv_fwd_desc, eng);
            ...
            auto conv_fwd_desc = convolution_forward::desc(
                ...
                x_md, w_md, b_md, y_md, {conv_stride, conv_stride}, padding, padding);
            conv_fwd_pd = convolution_forward::primitive_desc(conv_fwd_desc, eng);
            conv_fwd = convolution_forward(conv_fwd_pd);
        }
    ...
    void forward_save_context_prim(memory &x, std::vector<primitive> &prim, 
                                  std::vector<std::unordered_map<int, memory>> &args) {
        if (train_w) {
    ...
    void backward_data_prim(memory &gy, memory &gx, std::vector<primitive> &prim, 
                       std::vector<std::unordered_map<int, memory>> &args) {
    ...
    void backward_weights_prim(memory &gy, std::vector<primitive> &prim, 
                       std::vector<std::unordered_map<int, memory>> &args) {
...
public:
    ...
    bool train_w, train_x;
    int avg_rad;
    ConvOurs(engine &eng, memory::dims x_shape, int c_out,
         bool train_w, bool train_x,
         int conv_stride, int kern_size, int avg_rad): x_shape(x_shape), train_w(train_w),
         train_x(train_x), conv_stride(conv_stride), kern_size(kern_size), avg_rad(avg_rad) {
        int batch = x_shape[0], c_in = x_shape[1], x_h = x_shape[2], x_w = x_shape[3];
        ...
        if (train_w || train_x) {
            ...
            int p_h = ceil(y_shape[2] * 1.0 / avg_rad), p_w = ceil(y_shape[3] * 1.0 / avg_rad);
            memory::dims avg_gy_pad = {p_h * avg_rad - y_shape[2], p_w * avg_rad - y_shape[3]};
            ...
            if (train_w) {
                auto bwd_w_conv_desc = convolution_backward_weights::desc(
                    ...
                    {1, 1}, {0, 0}, {0, 0});
                bwd_w_conv_pd = convolution_backward_weights::primitive_desc(bwd_w_conv_desc, eng, conv_fwd_pd);
                ...
                reduce_x_pd = pooling_forward::primitive_desc(reduce_x_desc, eng);
            ...
            if (train_x) {
                auto bwd_x_conv_desc = convolution_backward_data::desc(
                    ...
                    {{batch, c_out, p_h, p_w}, dt::f32, tag::any},
                    {1, 1}, {0, 0}, {0, 0});
                bwd_x_conv_pd = convolution_backward_data::primitive_desc(bwd_x_conv_desc, eng, conv_fwd_pd);
                ...
                if (kern_size != 1) {
                    auto weight_sum_desc = reduction::desc(
                        ...
                        {{c_out, c_in, kern_size, kern_size}, dt::f32, tag::oihw}, 
                        {{c_out, c_in, 1, 1}, dt::f32, tag::any}, 0, 0);
                    weight_sum_pd = reduction::primitive_desc(weight_sum_desc, eng);
                    weight_sum_op = reduction(weight_sum_pd);
                    if (weight_sum_pd.src_desc() != conv_fwd_pd.weights_desc()) {
            ...
            auto conv_fwd_desc = convolution_forward::desc(
                prop_kind::forward_inference, algorithm::convolution_direct, 
                x_md, w_md, b_md, y_md, {conv_stride, conv_stride}, {1, 1}, {1, 1});
            conv_fwd_pd = convolution_forward::primitive_desc(conv_fwd_desc, eng);
            conv_fwd = convolution_forward(conv_fwd_pd);
            w = memory(conv_fwd_pd.weights_desc(), eng);
            b = memory(conv_fwd_pd.bias_desc(), eng);
    ...
    void forward_save_context_prim(memory &x, std::vector<primitive> &prim, 
        ...
        if (train_w) {
            ...
            args.push_back({{DNNL_ARG_SRC, x_reorder}, {DNNL_ARG_DST, x_reduced}});
            if (x_reduced.get_desc() != bwd_w_conv_pd.src_desc()) {
                prim.push_back(reorder(x_reduced, bwd_x));
                args.push_back({{DNNL_ARG_FROM, x_reduced}, {DNNL_ARG_TO, bwd_x}});
            }
        }
        if (train_x) {
            if (kern_size != 1) {
                if (w.get_desc() != weight_sum_pd.src_desc()) {
                    ...
                    args.push_back({{DNNL_ARG_FROM, w}, {DNNL_ARG_TO, w_reorder}});
                } else {
                    w_reorder = w;
                }
                prim.push_back(weight_sum_op);
                args.push_back({{DNNL_ARG_SRC, w_reorder}, {DNNL_ARG_DST, w_reduced}});
            }
            if (w_reduced.get_desc() != bwd_x_conv_pd.weights_desc()) {
                prim.push_back(reorder(w_reduced, bwd_w));
                args.push_back({{DNNL_ARG_FROM, w_reduced}, {DNNL_ARG_TO, bwd_w}});
            }
        }
    }
    void backward_filter_prim(memory &gy, std::vector<primitive> &prim,
                            std::vector<std::unordered_map<int, memory>> &args) {
        ...
        if (avg_gy.get_desc() != bwd_x_conv_pd.diff_dst_desc()) {
            ...
            args.push_back({{DNNL_ARG_FROM, avg_gy}, {DNNL_ARG_TO, avg_gy_reorder}});
        }
    }
    void backward_data_prim(memory &gy, memory &gx, std::vector<primitive> &prim,
                            std::vector<std::unordered_map<int, memory>> &args) {
        ...
        args.push_back({{DNNL_ARG_DIFF_DST, avg_gy_reorder}, {DNNL_ARG_DIFF_SRC, gx}, {DNNL_ARG_WEIGHTS, bwd_w}});
    }
    void backward_weights_prim(memory &gy, std::vector<primitive> &prim, 
                       std::vector<std::unordered_map<int, memory>> &args) {
        assert(gy.get_desc() == avg_gy_pd.src_desc());
...
void vanilla_conv() {
    ...
    std::vector<std::unordered_map<int, memory>> fwd_args, fwd_context_args, bwd_data_args, bwd_weights_args;
...
void new_conv() {
    ...
    std::vector<std::unordered_map<int, memory>> fwd_args, fwd_context_args, bwd_filter_args, bwd_data_args, bwd_weights_args;
    auto input = memory(c.conv_fwd_pd.src_desc(), eng);
    auto output = memory(c.conv_fwd_pd.dst_desc(), eng);
...
struct ProfileResult {
    float backward_data_time;
    float backward_weight_time;
};

void profile_vanilla_conv(ProfileResult &result,
    engine &eng, stream &s, int repeat, int warmup,
    int batch, int x_h, int x_w,
    int c_in, int c_out,
    int stride, int kern_size) {
    std::chrono::time_point<std::chrono::system_clock> start, end;

    auto c = Conv(eng, {batch, c_in, x_h, x_w}, c_out, true, true, stride, kern_size);
    std::vector<primitive> fwd, fwd_context, bwd_data, bwd_weights;
    std::vector<std::unordered_map<int, memory>> fwd_args, fwd_context_args, bwd_data_args, bwd_weights_args;
    auto input = memory(c.conv_fwd_pd.src_desc(), eng);
    auto output = memory(c.conv_fwd_pd.dst_desc(), eng);
    ...
    result.backward_data_time = duration_us(start, end) / repeat;
    ...
    result.backward_weight_time = duration_us(start, end) / repeat;
}

void profile_ours_conv(ProfileResult &result,
    engine &eng, stream &s, int repeat, int warmup,
    int batch, int x_h, int x_w,
    int c_in, int c_out,
    int stride, int kern_size, int avg_rad) {
    std::chrono::time_point<std::chrono::system_clock> start, end;

    auto c = ConvOurs(eng, {batch, c_in, x_h, x_w}, c_out, true, true, stride, kern_size, avg_rad);
    std::vector<primitive> fwd, fwd_context, bwd_filter, bwd_data, bwd_weights;
    std::vector<std::unordered_map<int, memory>> fwd_args, fwd_context_args, bwd_filter_args, bwd_data_args, bwd_weights_args;
    auto input = memory(c.conv_fwd_pd.src_desc(), eng);
    auto output = memory(c.conv_fwd_pd.dst_desc(), eng);
...
int main(int argc, char** argv) {
    ...
    std::cin >> repeat >> warmup;
    // printf("Warmup: %d Repeat: %d\n", warmup, repeat);
    std::cin >> c_in >> c_out >> stride >> kern_size >> batch >> x_h >> x_w >> avg_rad;
    ...
    if (avg_rad < 1) {
        profile_vanilla_conv(result, eng, s, repeat, warmup, batch, x_h, x_w, c_in, c_out, stride, kern_size);
    } else {
        profile_ours_conv(result, eng, s, repeat, warmup, batch, x_h, x_w, c_in, c_out, stride, kern_size, avg_rad);
    }
    std::cout << result.backward_data_time << " " << result.backward_weight_time << " ";
    std::cout << std::endl;
    // printf("Done Sleep 100 ms\n");
