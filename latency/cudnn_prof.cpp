...
static int checkCudaError(cudaError_t code, const char* expr, const char* file,
                          int line) {
    if (code) {
        printf("CUDA error at %s:%d, code=%d (%s) in '%s'", file, line,
...
int convFwd(cudnnHandle_t handle_, float* devPtrX, float* devPtrW,
            float* devPtrY, cudnnTensorDescriptor_t cudnnXdesc,
    ...
    checkCudnnErr(cudnnGetConvolutionForwardWorkspaceSize(
        handle_, cudnnXdesc, cudnnWdesc, cudnnConvDesc, cudnnYdesc, algo,
        &workSpaceSize));
    if (workSpaceSize > 0) {
        checkCudaErr(cudaMalloc(&workSpace, workSpaceSize));
    }
    checkCudnnErr(cudnnConvolutionForward(
        handle_, (void*)(&alpha), cudnnXdesc, devPtrX, cudnnWdesc, devPtrW,
        cudnnConvDesc, algo, workSpace, workSpaceSize, (void*)(&beta),
        cudnnYdesc, devPtrY));
...
int convBwdW(cudnnHandle_t handle_, float* devPtrX, float* devPtrW,
             float* devPtrY, cudnnTensorDescriptor_t cudnnXdesc,
             cudnnFilterDescriptor_t cudnnWdesc,
             cudnnTensorDescriptor_t cudnnYdesc,
             cudnnConvolutionDescriptor_t cudnnConvDesc, float alpha,
             float beta, int wSize, bool sync) {
    ...
    checkCudnnErr(cudnnGetConvolutionBackwardFilterWorkspaceSize(
        handle_, cudnnXdesc, cudnnYdesc, cudnnConvDesc, cudnnWdesc, algo,
        &workSpaceSize));
    ...
    checkCudnnErr(cudnnConvolutionBackwardFilter(
        handle_, (void*)(&alpha), cudnnXdesc, devPtrX, cudnnYdesc, devPtrY,
        cudnnConvDesc, algo, workSpace, workSpaceSize, (void*)(&beta),
        cudnnWdesc, devPtrW));
    if (sync) checkCudaErr(cudaDeviceSynchronize());
...
int convBwdX(cudnnHandle_t handle_, float* devPtrX, float* devPtrW,
             float* devPtrY, cudnnTensorDescriptor_t cudnnXdesc,
             cudnnFilterDescriptor_t cudnnWdesc,
             cudnnTensorDescriptor_t cudnnYdesc,
             cudnnConvolutionDescriptor_t cudnnConvDesc, float alpha,
             float beta, int xSize, bool sync) {
    ...
    checkCudnnErr(cudnnGetConvolutionBackwardDataWorkspaceSize(
        handle_, cudnnWdesc, cudnnYdesc, cudnnConvDesc, cudnnXdesc, algo,
        &workSpaceSize));
    if (workSpaceSize > 0) {
        checkCudaErr(cudaMalloc(&workSpace, workSpaceSize));
    }
    ...
    checkCudnnErr(cudnnConvolutionBackwardData(
        handle_, (void*)(&alpha), cudnnWdesc, devPtrW, cudnnYdesc, devPtrY,
        cudnnConvDesc, algo, workSpace, workSpaceSize, (void*)(&beta),
        cudnnXdesc, devPtrX));
...
int avgConv(cudnnHandle_t handle_, int xShape[], int wShape[], int yShape[],
            int convStride[], cudnnTensorDescriptor_t xDesc,
            cudnnFilterDescriptor_t rawWdesc, cudnnTensorDescriptor_t yDesc,
            float* devPtrX, float* devPtrY, float* devPtrW, int wStride[],
    ...
    for (int dim = 0; dim < 2; dim++) {
        ...
        avgXshape[dim + 2] =
            std::ceil(xShape[2 + dim] * 1.0 / xPoolingSize[dim]);
        xPad[dim] = std::ceil(
            (avgXshape[dim + 2] * xPoolingSize[dim] - xShape[dim + 2]) / 2.0);
        avgGradYshape[dim] = yShape[dim];
        avgGradYshape[dim + 2] =
            std::ceil(yShape[2 + dim] * 1.0 / gradFilterSize[dim]);
        gradYpad[dim] = std::ceil(
            (avgGradYshape[dim + 2] * gradFilterSize[dim] - yShape[dim + 2]) /
            2.0);
    ...
    cudnnSetConvolutionNdDescriptor(cudnnConvDesc, 2, fwdConvPad, convStride,
                                    fwdConvDilation, CUDNN_CONVOLUTION,
                                    CUDNN_DATA_FLOAT);
    cudnnSetTensorNdDescriptor(avgGradYdesc, CUDNN_DATA_FLOAT, 4, avgGradYshape,
                               avgGradYstride);
    cudnnSetTensorNdDescriptor(avgXdesc, CUDNN_DATA_FLOAT, 4, avgXshape,
                               avgXstride);
    cudnnSetTensorNdDescriptor(sumWdesc, CUDNN_DATA_FLOAT, 4, sumWshape,
                               sumWstride);
    cudnnSetFilterNdDescriptor(sumWfiltDesc, CUDNN_DATA_FLOAT,
                               CUDNN_TENSOR_NCHW, 4, sumWshape);
    ...
    cudnnSetReduceTensorDescriptor(
        wReduceDesc, cudnnReduceTensorOp_t::CUDNN_REDUCE_TENSOR_ADD,
    ...
    cudnnSetPooling2dDescriptor(
        ...
        cudnnNanPropagation_t::CUDNN_NOT_PROPAGATE_NAN, gradFilterSize[0],
        gradFilterSize[1], gradYpad[0], gradYpad[1], gradFilterSize[0],
        gradFilterSize[1]);
    cudnnGetPooling2dForwardOutputDim(gradYpoolingDesc, yDesc, &tempShape[0],
                                      &tempShape[1], &tempShape[2],
                                      &tempShape[3]);
    for (int dim = 0; dim < 4; dim++) {
    ...
    cudnnSetPooling2dDescriptor(
        xPoolingDesc,
        cudnnPoolingMode_t::CUDNN_POOLING_AVERAGE_COUNT_EXCLUDE_PADDING,
        cudnnNanPropagation_t::CUDNN_NOT_PROPAGATE_NAN, xPoolingSize[0],
        xPoolingSize[1], xPad[0], xPad[1], xPoolingSize[0], xPoolingSize[1]);
    cudnnGetPooling2dForwardOutputDim(xPoolingDesc, xDesc, &tempShape[0],
                                      &tempShape[1], &tempShape[2],
                                      &tempShape[3]);
    for (int dim = 0; dim < 4; dim++) {
    ...
    cudnnSetConvolution2dDescriptor(gradY2gradXConvDesc, 0, 0, convStride[0],
                                    convStride[1], 1, 1,
    ...
    for (int iter = 0; iter < repeat; iter++) {
        convFwd(handle_, devPtrX, devPtrW, devPtrY, xDesc, rawWdesc, yDesc,
                cudnnConvDesc, alpha, beta, ySize, false);
        cudnnGetReductionWorkspaceSize(handle_, wReduceDesc, wDesc, sumWdesc,
                                       &workSpaceSize);
        if (workSpaceSize > 0) {
            checkCudaErr(cudaMalloc(&workSpace, workSpaceSize));
        }
        checkCudnnErr(cudnnReduceTensor(
            handle_, wReduceDesc, nullptr, 0, workSpace, workSpaceSize,
            (void*)&alpha, wDesc, devPtrW, (void*)&beta, sumWdesc, devSumW));
        if (workSpaceSize > 0) {
            checkCudaErr(cudaFree(workSpace));
        }
        checkCudnnErr(cudnnPoolingForward(handle_, xPoolingDesc, (void*)&alpha,
                                          xDesc, devPtrX, (void*)&beta,
                                          avgXdesc, devAvgX));
    ...
    start = std::chrono::high_resolution_clock::now();
    for (int iter = 0; iter < repeat; iter++) {
        checkCudnnErr(cudnnPoolingForward(
            handle_, gradYpoolingDesc, (void*)&alpha, yDesc, devPtrY,
            (void*)&beta, avgGradYdesc, devAvgGradY));
        cudnnGetConvolutionBackwardDataWorkspaceSize(
            handle_, sumWfiltDesc, avgGradYdesc, gradY2gradXConvDesc, avgXdesc,
        ...
        checkCudnnErr(cudnnConvolutionBackwardData(
            handle_, (void*)&alpha, sumWfiltDesc, devSumW, avgGradYdesc,
            devAvgGradY, gradY2gradXConvDesc,
            cudnnConvolutionBwdDataAlgo_t::CUDNN_CONVOLUTION_BWD_DATA_ALGO_1,
            workSpace, workSpaceSize, (void*)&beta, avgXdesc, devAvgX));
        if (workSpaceSize > 0) {
            checkCudaErr(cudaFree(workSpace));
        }

        cudnnGetConvolutionBackwardFilterWorkspaceSize(
            handle_, avgXdesc, avgGradYdesc, gradY2gradXConvDesc, sumWfiltDesc,
        ...
        cudnnConvolutionBackwardFilter(
            handle_, (void*)&alpha, avgXdesc, devAvgX, avgGradYdesc,
            devAvgGradY, gradY2gradXConvDesc,
            cudnnConvolutionBwdFilterAlgo_t::
                CUDNN_CONVOLUTION_BWD_FILTER_ALGO_1,
            workSpace, workSpaceSize, (void*)&beta, sumWfiltDesc, devSumW);
        if (workSpaceSize > 0) {
            checkCudaErr(cudaFree(workSpace));
        }
        checkCudaErr(cudaDeviceSynchronize());
...
clean:
    ...
    if (gradY2gradXConvDesc)
        cudnnDestroyConvolutionDescriptor(gradY2gradXConvDesc);
    return numErrors;
}

int baseConv(cudnnHandle_t handle_, int xShape[], int wShape[], int yShape[],
             int convStride[], cudnnTensorDescriptor_t xDesc,
             cudnnFilterDescriptor_t wDesc, cudnnTensorDescriptor_t yDesc,
             float* devPtrX, float* devPtrY, float* devPtrW, int wStride[],
             int repeat, ProfileResult& result) {
    std::chrono::time_point<std::chrono::system_clock> start, end;
    ...
    checkCudnnErr(cudnnSetConvolutionNdDescriptor(
        cudnnConvDesc, 2, xPad, convStride, dilation, CUDNN_CONVOLUTION,
        CUDNN_DATA_FLOAT));

    start = std::chrono::high_resolution_clock::now();
    for (int iter = 0; iter < repeat; iter++) {
        convFwd(handle_, devPtrX, devPtrW, devPtrY, xDesc, wDesc, yDesc,
                cudnnConvDesc, 0.8, 0.0, ySize, true);
        end = std::chrono::high_resolution_clock::now();
    }
    result.forward_start_timestamp = to_us(start.time_since_epoch());
    result.forward_time = duration_ms(start, end) / repeat;
    ...
    start = std::chrono::high_resolution_clock::now();
    for (int iter = 0; iter < repeat; iter++) {
        convBwdW(handle_, devPtrX, devPtrW, devPtrY, xDesc, wDesc, yDesc,
                 cudnnConvDesc, 0.8, 0.0, wSize, false);
        convBwdX(handle_, devPtrX, devPtrW, devPtrY, xDesc, wDesc, yDesc,
                 cudnnConvDesc, 0.8, 0.0, xSize, false);
        checkCudaErr(cudaDeviceSynchronize());
    }
    end = std::chrono::high_resolution_clock::now();
    result.backward_start_timestamp = to_us(start.time_since_epoch());
...
int profile_conv(cudnnHandle_t handle_, int cIn, int cOut, int stride,
                 int kernSize, int batch, int xH, int xW, int avgRad,
                 int warmup, int repeat, ProfileResult& result) {
    int numErrors = 0;
    ...
    checkCudnnErr(cudnnSetTensorNdDescriptor(xDesc, CUDNN_DATA_FLOAT, 4, xShape,
                                             strideX));
    checkCudnnErr(cudnnSetTensorNdDescriptor(yDesc, CUDNN_DATA_FLOAT, 4, yShape,
                                             strideY));
    checkCudnnErr(cudnnSetFilterNdDescriptor(wDesc, CUDNN_DATA_FLOAT,
                                             CUDNN_TENSOR_NCHW, 4, wShape));

    initImage((int8_t*)hostPtrX, xSize);
    initImage((int8_t*)hostPtrW, wSize);
    initImage((int8_t*)hostPtrY, ySize);
    checkCudaErr(cudaMemcpy(devPtrX, hostPtrX, sizeof(hostPtrX[0]) * xSize,
                            cudaMemcpyHostToDevice));
    checkCudaErr(cudaMemcpy(devPtrW, hostPtrW, sizeof(hostPtrW[0]) * wSize,
                            cudaMemcpyHostToDevice));
    checkCudaErr(cudaMemcpy(devPtrY, hostPtrY, sizeof(hostPtrY[0]) * ySize,
                            cudaMemcpyHostToDevice));
    checkCudaErr(cudaDeviceSynchronize());

    if (avgRad < 1) {
        numErrors +=
            baseConv(handle_, xShape, wShape, yShape, convStride, xDesc, wDesc,
                     yDesc, devPtrX, devPtrY, devPtrW, strideW, warmup, result);
        numErrors +=
            baseConv(handle_, xShape, wShape, yShape, convStride, xDesc, wDesc,
                     yDesc, devPtrX, devPtrY, devPtrW, strideW, repeat, result);
    } else {
        numErrors += avgConv(handle_, xShape, wShape, yShape, convStride, xDesc,
                             wDesc, yDesc, devPtrX, devPtrY, devPtrW, strideW,
                             avgRad, warmup, result);
        numErrors += avgConv(handle_, xShape, wShape, yShape, convStride, xDesc,
                             wDesc, yDesc, devPtrX, devPtrY, devPtrW, strideW,
                             avgRad, repeat, result);
    }
clean:
...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        ...
        while (cfg_file >> c_in >> c_out >> stride >> kern_size >> batch >>
            ...
            profile_conv(handle_, c_in, c_out, stride, kern_size, batch, x_h,
                         x_w, avg_rad, warmup, repeat, result);
